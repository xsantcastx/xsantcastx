 ...
 *
 * The Godforge keeps its state in seven independent localStorage blobs, each
 ...
 * is why adding cloud save did not require rewriting seven services.
 *
 ...
 * The six blobs below have no such seam, and inventing one for each would have
 * been six rewrites to reach the same place.
 *
 * ─────────────────────────────────────────────────────────────────────────────
...
export interface SyncedBlob {
   ...
   * question. Only the discovery-date ledger needs one.
   */
 ...
 *
 * A per-blob merge function for each of the six was the obvious alternative and
 * the wrong one: they would have had to be kept in step with six state shapes
 * owned by six other files, and the failure mode of falling behind is silent
