...
export const VERSION_HISTORY: VersionRelease[] = [
  {
    ...
    date: '2026-08-29',
    highlights: [
      'The Market\'s cheapest thing was 5,000 Gold and a new Keeper starts with none, mints 1 a strike and 0.1 a second. That is four and a half thousand strikes, or fourteen hours of waiting, before the shop sells its owner anything — and a shop whose entire stock is unaffordable on a first visit does not read as a ladder, it reads as a wall. The first Forge Bellows is free now. Only the first: level two is still 6,250 and every other rung on every other ladder is exactly where it was, so what changed is the doorway, not the curve behind it. Income goes 0.1 to 0.6 a second, which is a sixfold jump the header counter shows while you are looking at it, and the first visit now ends with the visitor owning something',
      'The free two-minute Scout was always the real answer and nothing on the Market said so. It costs nothing, the two starter explorers are minted on a fresh save, and it comes home with 5,000-10,000 Gold — the tutorial\'s fourth screen has pointed at it under "Do this first" since the tutorial shipped. But somebody who skipped the tutorial arrives at the shop with an empty purse and no signpost at all, so the shop now carries one, shown to exactly the visitor in that state: holding nothing off any Gold ladder and unable to afford the cheapest paid rung. Both halves are needed. The purse alone would put "you already have the explorers" in front of a veteran who has just spent down to nothing',
      'A free rung had to survive a sync, and two separate gates would have eaten it. A zero-cost purchase records buy-upgrade with amount 0, and both parseOp and applyOp carried a blanket "greater than zero" rule — so the rung would have lived on the device that bought it and vanished the first time the ledger came back through the cloud merge, which reads every op through both. There is a ZERO_AMOUNT_OK set now, scoped to that one kind; every other amount-carrying op still has to be positive. A unit test caught the second gate after only the first had been widened, which is the whole argument for having written it',
      'The Market had been quoting a price it does not charge. Each Gold card was priced with costOf(baseCost, owned) straight out of the catalogue while EconomyService charges nextCost(), which applies the Charisma discount — and that number is not only what the visitor reads, it is shipped back as expectedCost and compared with !== by purchaseListing, which refuses the whole purchase as stale on any mismatch. So every Gold ladder in the shop was unbuyable for exactly the players who had earned a discount. The card reads its price off the service now, so display, quote and charge cannot disagree',
      'Every native select on the site was 25 pixels tall on an iPhone. Nine of them — three on the Bench, two in the Rune Forge, two in the Collection Log, one on the Codex, one on the Market — each styled with a min-height at or near the 44px touch floor, and each measuring under half of it in WebKit. It is not a specificity problem, which is the part that costs an hour: the rule matches, and getComputedStyle still reports 18px, because a select left at appearance:auto is a menulist and WebKit substitutes its own metrics for the author\'s height and padding. Chromium honours them, which is how this survived a mobile pass, an accessibility pass and every audit anybody ran in one browser. There is a global appearance:none rule now, generalising the one select in the app that already set it and was therefore the only one measuring correctly',
      'The NPC is the third fixed widget to eat the Rune Forge\'s action row on a phone, after the Forge Flame and the discovery toast. At 375px the speech bubble measures 347 by 63 at y 581, and the roll buttons sit at y 586 — the same band, near enough exactly — so ×10, ×100, ALL and Stop all went to the bubble. It stands down under body.gf-reveal-open with the other two now. Worth knowing for the fourth: Angular compiles the component\'s own `.npc > *` rule to three class-weight terms, so the obvious override loses the specificity tie and silently does nothing',
      'NG0100 on every load of the Sanctum, and it was the only console error left anywhere on the site. The Forge View mounts its own flame, which claims the corner from a lifecycle hook — after the shell\'s @if had already been checked in that pass. The assignment waits for a microtask now, deliberately not a frame, because frames do not run in a background tab',
      'Thirty-one of the e2e suite\'s hundred-and-two specs had been failing on main for four releases and all thirty-one were one bug. The first-run tutorial raises a full-viewport modal for any browser with no ledger and no wallet, which is every browser Playwright opens; sixteen specs seed storage through their own init script and not one of them knew it existed. Every click resolved its target, scrolled to it, and then spent the full thirty-second timeout being told a div was in the way. The config seeds the tutorial\'s own record now, before any spec runs, so a spec written tomorrow inherits the fix. Chromium is a hundred and two of a hundred and two',
      'The suite also stopped grading other people\'s builds. It served dist on a hardcoded 4173 with reuseExistingServer on, so a concurrent worktree\'s preview server would answer the run — silently, and a green result proved nothing about the branch under test. PW_PORT overrides it; the default is unchanged, so CI is unchanged',
      'Five specs had outlived what they assert. One expected five Market pages when the catalogue has six, one expected nine shelf tabs when there are ten, one priced Seeker\'s Lens at 5 Essence when it has cost 500 since the hundredfold rebalance — that one refused both its purchases as stale and had been guarding nothing at all — and one was still the Angular CLI scaffold checking for "app is running!". Each derives its expectation now instead of naming a number, and the scaffold spec was replaced with one that checks something real: that every full-viewport overlay is a sibling of main rather than a descendant, which is the property routeFadeIn\'s transform breaks',
      'CLAUDE.md was 662 lines of operating manual for a developer-tools site that no longer exists — the cosmic engine, the galaxy map, 126 tool pages, the guestbook. Every route it named redirects. It is 265 lines about the game now: the vocabulary, the route map, the early-game curve and the three invariants holding the free rung up, the save-state rules, and the ten traps that have actually bitten. Every figure in it was checked against the repo',
    ],
  },
  {
    version: '2.88.0',
  ...
  },
  {
    ...
    date: '2026-08-25',
    highlights: [
      'The design system shipped an update and roughly half of it was worth taking. --violet was one variable doing two jobs: the Umbral realm\'s #a855f7 and the interaction colour every hover edge, focus ring and press glow on the site is painted in. They are close enough to look like a duplicate and they are not the same fact — one is a place, the other is a state — so a realm restyle would have repainted every button on every page. --violet is now #8b5cf6, the value src/styles/interactions.css has always used; --realm-umbral keeps #a855f7 and a comment saying not to collapse them again',
      '--gf-accent was the same bug with money on the line. styles.css publishes it as gold for value and reward — a price, an owned badge, a loot frame — and interactions.css re-declared it as violet. interactions.css loads last, so it won everywhere: a bank tile you had selected drew a violet border around gold text, and a loot figure that had just taken a hit outlined itself in violet over a gold wash. Interaction has its own name now, --gf-interactive, and the two roles stopped fighting. Measured before and after on Basalt Seamworks: a struck loot frame went from rgb(168,85,247) to rgb(232,200,152), which is the same gold as the XP figure beside it',
      'Rarity keeps two ladders instead of pretending it has one. EclipseRarity has six tiers and RuneTier has seven, both are persisted in saves and egg tables, and neither can be renamed without a data migration — so both are named: --eclipse-* and --rune-*, with --rarity-* aliasing the seven-tier one the components already take as a prop. The six map onto the seven by ladder role, which is what rarity.model.ts already documents, and the four Eclipse-only tier names picked up [data-rarity] scopes of their own. Every hex is read back out of the two model files, so the tokens cannot drift from the data',
      'The values the design system proposed for those ladders were not taken. They are a stale reading of this repo, from before the ladder moved onto the system, and its epic rung is one of the six golds retired in "One Gold" — which scripts/check-ds-adherence.js fails the build on, including when the hex appears in a comment. It caught this change\'s own explanatory paragraph, which is the rule working exactly as intended and is now noted in the file',
      '--glow-* stays a box-shadow string. The design system flattened all of them to bare rgba(), which reads like a tidy-up and is a silent breakage: offline-summary.component.css animates `box-shadow: var(--glow-legendary)` directly, and a bare colour there is an invalid declaration the browser drops with no error and no build warning',
      'Motion stopped keeping two answers. tokens/motion.css declared --ease-standard as cubic-bezier(.2,0,0,1) while every button, card and toast on the site animated on interactions.css\'s own cubic-bezier(.4,0,.2,1) literal. The token now holds the curve that actually paints, --dur-base / --dur-press / --dur-enter / --lift-hover / --press-scale / --ease-spring / --reveal-offset / --dur-shimmer name the rest of what that file had inlined, and interactions.css reads them — which also means reduced motion reaches the interaction layer through the variables rather than only through its own selector list',
      'The design system\'s reduced-motion block ends in a global `*, *::before, *::after { animation-duration: 1ms !important }`. Not adopted. This site\'s reduced-motion paths are hand-written to land on a static END state; an !important sweep outranks all of them and stops the animation where it happens to be instead — and the boot curtain is precisely the case where that leaves a full-viewport fixed layer on screen',
      'Semantic tokens picked up the values two real components had already settled on by hand: the Quick Inspect panel\'s body copy, muted labels, opaque #0c0d16 base, backdrop scrim and secondary button fill, and the bank grid\'s empty-slot well, 148px tile floor and 88px pad height. Sampling beats inventing here — those are the values the site already reads as. --text-muted\'s worst case tightens from 5.02:1 to 4.97:1 on --surface-card, still above the 4.5 AA floor, and body copy stays above 9:1 everywhere',
      'Three of the design system\'s changes were rejected outright and each has a comment saying why, because the next update will propose them again. Gold stays #e8c898 — the DS wants a retired one back and the adherence gate would have caught it anyway. Fonts stay self-hosted; the DS reverted to a Google Fonts @import, which is a render-blocking third-party request this app deliberately removed once already. And --z-sticky stays at 100 rather than dropping to 20, because 20 sat underneath the achievement toast and the Forge Flame, which is a collision that was fixed on purpose',
      '--nav-h, --header-offset and the Quick Inspect panel width are tokens now rather than a number every call site guessed at. The header still measures its real height into --nav-h at runtime and that inline write still outranks the token, which is the point: the token is the declared fallback, so removeProperty lands on 64px instead of on whichever number each consumer happened to write after the comma',
    ],
  },
  {
    version: '2.85.0',
    ...
    date: '2026-08-25',
    highlights: [
      'Come back after five minutes or more and a summary says what the forge did while the tab was shut: the Gold the ledger settled, the runes the shift turned up, the expeditions that came home, and whether the daily board and the Contract Board rolled over. Eight hours is the ceiling, which is the ceiling the ledger already used — two numbers here would have meant a screen reporting a span the Gold was not actually paid for',
      'The screen mints almost nothing. Four systems already know how to price time — the ledger settles Gold from its own lastIdleAt, the shift settles pulls from their own roll clocks, the expedition board settles landings from their own deadlines, the two boards roll on their own day keys — and a fifth computing any of it again would be a second answer the visitor could check against their balance and find wrong. The Gold line is the receipt the settlement left on its way past, not a recalculation',
      'Which is also why the button is an acknowledgement rather than a payout. Holding the rewards until somebody presses Claim is the shape that looks more honest and is not: it loses a night to a closed tab, loses it again to a crashed renderer, and lets two tabs on one forge each hold and each pay. Everything is banked before the curtain goes up, and the line under the button says so',
      'Thralls now settle an absence. The shift has always paused with the tab — a throttled timer measuring its own elapsed span would hand back a minute of pulls in one frame, and a pull spends Gold nobody was present to authorise — and both of those stop applying on a return, where the visitor is here and is shown exactly what was spent and what it bought. Bounded at a hundred pulls, because eight hours of a five-second Legendary is fifty-seven thousand of them, each a Gold spend and a rune grant, on the first frame of a page load',
      'The replay steps the clock to the next moment a Thrall is due rather than beating once a second: a hundred iterations instead of half a million. It breaks on an empty ledger too, and that guard is load-bearing rather than tidy — pull refuses a broke Thrall without stamping lastRollAt, which is right while the page is open and is an unbreakable loop in a replay that steps to the next due moment',
      'An overnight expedition now pays out wherever you come back to. Before this the board only settled when the Forge View mounted, so a Deep Dive that landed at 3am sat there until you happened to open the expedition panel. The chunk is still lazy and is fetched only when a raw read of the saved blob says something has actually landed — a save with nothing out never touches it',
      'Offline XP is new, and it is capped under presence rather than beside it. The Ambient Forge pays for visible minutes only and stops at thirty a day, so a quarter of an XP a minute across the eight-hour ceiling tops out at 120 — the same as a perfect day of being here on a thirty-day streak, and a quarter of what those eight hours would have paid with the tab open. Closing the tab is never the better way to earn',
      'The stamp is device-local and never walks backwards. Syncing it would mean a phone left open all afternoon told a laptop opened the next morning it had been away five minutes; taking the max on write means closing an old background tab cannot rewind it and manufacture an absence out of a session nobody left. Three writers — pagehide, visibility, and a thirty-second heartbeat — because the first is the one that matters and is not guaranteed',
      'A long absence on a save that earned nothing raises no curtain at all. Announcing zero of everything is the version of this feature that makes people close the tab, and a board rollover counts on its own because "your daily quests are back" is a reason to look',
      'Two bugs the browser found rather than the specs. The panel is 986px tall on a phone with every section filled, and a centred flex item taller than its scrolling container overflows above scrollTop 0 where no scroll can reach — the eyebrow and the first line of the title were simply gone. And the Gold count-up hung off a view query that is not always resolved when a createComponent-mounted view runs ngAfterViewInit, so the template now renders the settled number and the animation takes it back to zero to climb — the animation is the thing that can fail, not the number',
      'The whole thing costs the initial bundle about 7 kB. The summary, its stylesheet and its five sections are a lazy chunk pulled the first time somebody comes back to something, and the two settlements it triggers are dynamic imports behind a raw read of their own blobs',
    ],
  },
  {
    version: '2.84.0',
  ...
  },
  {
    ...
    highlights: [
      ...
      'The shared classes were added alongside the ep-* ones rather than replacing them, so the component keeps its own styling. Verified on built output: the badge now resolves to "Archivum" in rgb(200, 152, 104), matching the Verge badge on json-formatter',
