...
export const DEV_LOG: DevLogEntry[] = [
  {
    id: 'cloud-as-the-record',
    date: '2026-08-13',
    title: 'Moving the save into the cloud, and what "source of truth" actually costs',
    summary:
      'Nineteen services each owned their own localStorage key and cloud save copied the blobs around them. That put the record in the browser and the backup in the cloud, which is the wrong way round for anything that comes next.',
    category: 'architecture',
    tags: ['Cloud Save', 'Architecture', 'Firestore', 'Storage', 'Offline'],
    body: [
      'The save worked. It was also shaped so that every future feature would have to go around it: nineteen services each read and wrote their own localStorage key, and cloud save copied those blobs up and down from the outside. Anything that wants to read a visitor\'s state from somewhere that is not this browser — a leaderboard, a Grand Exchange, a guild roster — would have had to ask nineteen services for their private copies. So the record moved to Firestore and the browser became its cache.',
      'The first thing to settle was what "source of truth" can mean in a client that has to paint immediately. Reads cannot go to the network: these services hydrate on first injection, the header\'s XP bar and the currency rail read during first paint, and the Firestore SDK is 450 kB deliberately kept out of the initial bundle. Making reads await it would put a network hop in front of the first frame of every page — including the 126 tool pages whose visitors are overwhelmingly anonymous and will never sign in. So reads stay synchronous against the cache, and authority is asserted at a different moment: signing in pulls the account, reconciles it, writes the result to the cache and pushes it into the live services. After that the cache *is* the cloud copy, byte for byte, and reading it is reading the cloud.',
      'The brief asked for writes to reach Firestore first and localStorage second. They do the reverse, and the brief\'s own next line is the reason — cloud writes are debounced and batched, and a debounced write cannot also be the first one. The deeper reason is that awaiting the network before recording a purchase would lose it if the tab closed mid-round-trip, and would make `buyUpgrade()` and every one of its callers async to buy nothing. Which store is written first is not what authority means; which one wins when they disagree is, and that is settled by the pull.',
      'The pull reads both subcollections whole rather than nineteen documents one at a time: same billed reads, two round trips instead of nineteen, on a path that runs on every page load of every signed-in visitor. The push then reuses what the pull just read instead of asking again — without that, signing in paid for the account twice, once to read it and once to re-read every document it was about to write.',
      'Writes are batched into one commit. That is what makes the ledger affordable: idle Gold settles once a second and the Flame can be struck twice a second, so a write-per-change would be a document write per second for the life of the tab. A four-second debounce with a twenty-second ceiling collapses a burst of clicking into one commit while still syncing three times a minute during sustained play.',
      'Two details are load-bearing and neither is obvious. A batch is atomic, so a single refused document fails all of them — and this database refuses writes by design, because the rule on the progression document enforces that XP may never go down. Left alone, one legitimately-refused write would hold eighteen unrelated blobs hostage indefinitely, so a failed batch retries per document and the refusal is isolated to the document that earned it. And the progression document is stored flat while every other blob is wrapped in an envelope, because that same rule reads a *top-level* `xp`: wrapping it would move the field to `v.xp`, the rule\'s own `!(\'xp\' in ...)` escape would pass every write unchecked, and the guard would be gone with nothing to show for it.',
      'Two bugs surfaced that were not the point of the change. Progression stamps a fresh `updatedAt` on every merge, so comparing the serialised copies reported a change on every single pull — previously worth one gratuitous page reload per session, and here it would have been a Firestore write on every page load of every signed-in visitor, forever. The comparison now ignores declared volatile fields. And progression\'s hand-written merge takes the *earlier* `createdAt` where the structural rule takes the greater string; routing it through the generic path would have walked every visitor\'s "first struck" date forward a little at a time, which is the kind of corruption nobody notices for months.',
      'The test harness produced the best find of the day. The two-device test wiped localStorage and then built the second device — and it kept starting with the first device\'s Gold already on disk. `TestBed.resetTestingModule()` destroys the providers, `EconomyService` implements `OnDestroy` by flushing its ledger, and so tearing down device A wrote device A\'s save back over the wipe. It is the same shape as the bug the previous release fixed, in the harness rather than the product, and it would have made the test pass while proving nothing. Two real browsers cannot do that to each other; a test runner can, and did.',
      'Offline is a supported state now rather than a failure. The write queue holds, the link reports itself honestly, retries back off to a minute, and the queue drains on reconnect or on the tab coming back. Signed out, none of it runs: no SDK is fetched, no request is made, and the game is the same local-only one it has always been.'
    ]
  },
  {
