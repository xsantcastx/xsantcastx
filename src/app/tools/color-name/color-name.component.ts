...
export class ColorNameComponent {
  ...
  readonly twitterShareUrl = `https://twitter.com/intent/tweet?text=${encodeURIComponent('Free CSS Color Name Finder — find the closest named CSS color from any hex value')}&url=${encodeURIComponent(SITE_URL + '/tools/color-name')}`;
  readonly linkedInShareUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(SITE_URL + '/tools/color-name')}`;

  hexInput = '#663399';
  searchQuery = '';
  closestColor: NamedColor | null = null;
  distance = 0;
  copied = false;

  readonly colors: NamedColor[] = [
  ...
  constructor(private router: Router) { this.findClosest(); }
  ...
  private findClosest(): void {
    const input = this.hexToRgb(this.hexInput);
    if (!input) return;
    ...
    for (const c of this.colors) {
      ...
      if (!rgb) continue;
      const d = Math.sqrt((input.r - rgb.r) ** 2 + (input.g - rgb.g) ** 2 + (input.b - rgb.b) ** 2);
      if (d < minDist) { minDist = d; closest = c; }
    }
    this.closestColor = closest;
    this.distance = Math.round(minDist);
    if (closest && closest.name === 'rebeccapurple') {
      this.eggs.trigger('color-rebecca');
    }
  }
  ...
  get filteredColors(): NamedColor[] {
    if (!this.searchQuery.trim()) return this.colors;
    const q = this.searchQuery.toLowerCase();
    return this.colors.filter(c => c.name.toLowerCase().includes(q) || c.hex.toLowerCase().includes(q));
  }

  selectColor(c: NamedColor): void {
    this.hexInput = c.hex;
    this.findClosest();
  }

  async copyName(): Promise<void> {
    if (!this.closestColor || !this.isBrowser) return;
    try { await navigator.clipboard.writeText(this.closestColor.name); this.copied = true; setTimeout(() => (this.copied = false), 2000); } catch {}
  }
}
